...
export const fetchBooks = (limit?: number) => {
  return async (dispatch: Dispatch<AppActions>) => {
    return axios.get(API_BOOKS_URL, { params: { limit } }).then(({ data }) => {
      return dispatch({
...
export const fetchPersons = (limit?: number) => {
  return async (dispatch: Dispatch<AppActions>) => {
    return axios
      .get(API_PERSONS_URL, { params: { limit } })
      .then(({ data }) => {
        return dispatch({
...
export const fetchQuizzes = (limit?: number) => {
  return async (dispatch: Dispatch<AppActions>) => {
    return axios.get(API_QUIZ_URL, { params: { limit } }).then(({ data }) => {
      return dispatch({
